'use strict';
global.PACKAGE_NAME = "BetfairGames";
...
for(let route in API) {
    app.post(`/api/${PACKAGE_NAME}/${route}`, _(function* (req, res) {
        ...
        try {
            let xmlResponse = yield API[route](req, res),
                response    = yield lib.xml(xmlResponse);

            r.contextWrites['to'] = response[Object.keys(response)[0]]
            r.callback            = 'success';
        } catch(e) {
            r.callback            = 'error';
